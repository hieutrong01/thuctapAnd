...
public class NoteActivity extends BaseActivity {
    ...
    private void init() {
        ...
        //các giá trị  sẽ được lấy ra từ Intent đang xử lý và gán cho các biến group_name, current_doc_name, và text.
        group_name = getIntent().getStringExtra("group_name");
        current_doc_name = getIntent().getStringExtra("current_doc_name");
        text = getIntent().getStringExtra("note");
        // Các thuộc tính của richEditor như kích thước chữ, padding, và nội dung văn bản gán mặc định
        richEditor.setEditorFontSize(18);
        richEditor.setPadding(10, 10, 10, 10);
        richEditor.setHtml(text);
        richEditor.requestFocus();//, nghĩa là cho phép người dùng nhập văn bản hoặc chỉnh sửa nội dung trong richEditor.
    ...
    private void bindView() {
        ...
        findViewById(R.id.iv_save_note).setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                BaseActivity.hideKeyboard(NoteActivity.this);//để ẩn bàn phím
                dbHelper.updateGroupListDocNote(group_name, current_doc_name, richEditor.getHtml());//để cập nhật văn bản ghi chú
                finish();//để kết thúc hoạt động của NoteActivity.
            }
        });
        ...
        findViewById(R.id.ly_red_text).setOnClickListener(new View.OnClickListener() {
            private boolean isChanged;//khởi tạo biến cục bộ isChanged

            @Override
            public void onClick(View view) {
                //Nếu isChanged là true, gọi hàm richEditor.setTextColor(ViewCompat.MEASURED_STATE_MASK) để đặt màu văn bản
                //Nếu isChanged là false, gọi hàm richEditor.setTextColor(SupportMenu.CATEGORY_MASK) để đặt màu văn bản
               // Cuối cùng, gán isChanged = !isChanged để đảo trạng thái của biến isChanged
                //cho phép chúng ta thay đổi màu của văn bản với mỗi lần nhấn vào view ly_red_text.
